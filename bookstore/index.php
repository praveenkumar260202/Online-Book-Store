...
if(isset($_SESSION['id'])){
	...
	echo '<form class="hf" action="logout.php"><input class="hi" id="logout" type="submit" name="submitButton" value="Logout"></form>';
	echo '<form class="hf" action="edituser.php"><input class="hi" id="editpro" type="submit" name="submitButton" value="Edit Profile"></form>';
	echo '</blockquote>';
	echo '</header>';
}
...
if(!isset($_SESSION['id'])){
	...
	echo '<blockquote>';
	echo '<a href="index.php"><img src="image/logo.png"></a>';
	echo '<form class="hf" action="Register.php"><input class="hi" id="register" type="submit" name="submitButton" value="Register"></form>';
	echo '<form class="hf" action="login.php"><input class="hi" id="log" type="submit" name="submitButton" value="Login"></form>';
	echo '</blockquote>';
	echo '</header>';
}
echo '<blockquote>';
    ...
    while($row = $result->fetch_assoc()) {
	    ...
	    echo "<table>";
	   	...
	   	Quantity: <input id="'.$row['BookID'].'" type="number" value="1" name="quantity" style="width: 20%"/><br>
	   	<input type="hidden" value="'.$row['BookID'].'" name="ac"/>
	   	<input id="submits" class="button" type="submit" value="Add to Cart"/>
	   	</form></td></tr>';
	   	echo "</table>";
	   	echo "</td>";
    }
    ...
    echo "Total: <b>Rs".$total."</b><center><form action='checkout.php' method='post'><input id='checkout' class='button' type='submit' name='checkout' value='CHECKOUT'></form></center>";
    echo "</td></tr>";
	echo "</table>";
	echo '</blockquote>';
?>
